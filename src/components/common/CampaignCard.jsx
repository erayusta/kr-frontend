...
const CampaignCard = ({ image, title, brands, id, endDate, end_date, slug }) => {
  ...
  if (!mounted) {
    return (<Card key={id} className="md:max-w-md max-w-sm shadow-md hadow-lg hover:shadow-xl transition-transform duration-300 ease-in-out hover:-translate-y-1">
      <div className="relative group:">
        ...
        <Button className=" bottom-5 right-5 absolute bg-black/30 hover:bg-orange-500 rounded-full p-1 shadow-md" size="icon" variant="ghost">
          <HeartIcon className="h-4 w-4 text-white" />
          <span className="sr-only">Add to Favorites</span>
        </Button>
      </div>
      <div className="px-2 py-2">
        <Link title={title} href={`/kampanya/${slug}`}><h3 className="text-sm text-blue-950 mt-2 h-[75px]">{title}</h3></Link>  
        <Button asChild variant="outline" className="w-full rounded-b-lg">
          <Link title={title} href={`/kampanya/${slug}`}> Bilgi Al <ChevronRight size={18}></ChevronRight></Link>
        </Button>
      </div>
    </Card>);
  }
 ...
 return (<Card key={id} className="md:max-w-md max-w-sm shadow-md hadow-lg hover:shadow-xl transition-transform duration-300 ease-in-out hover:-translate-y-1">
  ...
  <div className="relative group:">
   ...
   <div className={`absolute top-2 right-4 ${remaining < 5 ? 'bg-red-500' : 'bg-primary'} shadow-md text-gray-50 px-2 py-1 rounded-md text-xs font-medium`}>
    <Clock10Icon className="mr-1 inline-block h-4 w-4" />
    <span>{remaining < 0 ? 'Süresi Doldu' : `${remaining} gün kaldı`}</span>
   </div>
   {brands && brands[0] && (
   <Link href={`/marka/${brands[0].slug}`}>
    <div className="absolute top-2 left-4 bg-white rounded-full p-2 shadow-md">
