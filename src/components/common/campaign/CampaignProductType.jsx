...
import { ShoppingCart, Star, Package, Truck, Shield, CheckCircle } from 'lucide-react';
...
export default function CampaignProductType({ campaign }) {
  ...
  const lowestPrice = stores.length > 0 
    ? Math.min(...stores.filter(s => s.price).map(s => parseFloat(s.price)))
    : productData.price;
  ...
  const getImageUrl = (image) => {
    if (!image) return 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=800';
            ...
            <div className="bg-gray-50 p-6">
              <div className="space-y-4">
                ...
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  <h3 className="font-semibold text-orange-900 mb-2">📢 Kampanya Detayları</h3>
                  <p className="text-sm text-gray-700">
                    {campaign.description || 'Bu ürün özel kampanya fiyatıyla sizlerle!'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                {productData.brand && (
                  <Badge variant="secondary" className="mb-2">
                    {typeof productData.brand === 'string' ? productData.brand : productData.brand.name}
                  </Badge>
                )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm text-gray-500">4.5 (127 değerlendirme)</span>
                </div>
              </div>
              ...
              <div className="border-t border-b py-4">
                <div className="flex items-end gap-3">
                  ...
                  {productData.original_price && productData.original_price > lowestPrice && (
              ...
              {productData.attributes && Object.keys(productData.attributes).length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(productData.attributes).slice(0, 6).map(([key, value]) => (
      ...
      {stores.length > 0 && (
        <Card>
          <CardContent className="p-6">
            <h3 className="font-semibold text-lg mb-4">Fiyat Karşılaştırması</h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-3 px-4 text-sm font-medium text-gray-700">Mağaza</th>
                    <th className="text-center py-3 px-4 text-sm font-medium text-gray-700">Stok</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-gray-700">Fiyat</th>
                    <th className="text-center py-3 px-4 text-sm font-medium text-gray-700"></th>
                  </tr>
                </thead>
                <tbody>
                  {stores.map((store, index) => {
                    const isLowest = store.price && parseFloat(store.price) === lowestPrice;
                    return (
                      <tr key={index} className="border-b hover:bg-gray-50 transition-colors">
                        ...
                        <td className="text-center py-3 px-4">
                          {store.in_stock !== undefined ? (
                            <Badge 
                              ...
                              className={cn(
                                "text-xs",
                                store.in_stock ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"
                              )}
                            >
                        ...
                        <td className="text-right py-3 px-4">
                          <div>
                            <span className={cn(
                              "font-bold",
                              isLowest ? "text-green-600 text-lg" : "text-gray-900"
                            )}>
                              {formatPrice(store.price)}
                            </span>
                            {isLowest && (
                              <div className="text-xs text-green-600 mt-1">En düşük fiyat</div>
                            )}
                          </div>
                        </td>
                        <td className="text-center py-3 px-4">
                          {store.url && (
                            <Button
                              ...
                              className={isLowest ? "bg-orange-600 hover:bg-orange-700" : ""}
                            >
