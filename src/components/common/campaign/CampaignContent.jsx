...
import { Info, ChevronRight, Calendar, ExternalLink } from "lucide-react";
...
export default function CampaignContent({ campaign, ads }) {
  ...
  // Debug için campaign verisini konsola yazdıralım
  useEffect(() => {
    console.log('Campaign Data:', campaign);
    console.log('Item Type:', campaign?.item_type);
    console.log('Product:', campaign?.product);
    console.log('Car:', campaign?.car);
    console.log('Real Estate:', campaign?.real_estate || campaign?.realEstate);
  }, [campaign]);
    ...
    if (contentRef.current) {
      ...
      images.forEach(img => {
        img.onerror = function() {
          ...
          } else if (this.src.includes('/campains/') && !this.src.includes('/uploads/')) {
            this.src = this.src.replace('/campains/', '/campaigns/');
          } else if (this.src.includes('/campaigns/') && !this.src.includes('/uploads/')) {
            this.src = this.src.replace('/campaigns/', '/campaigns/uploads/');
          } else {
            // Show placeholder or hide
            this.style.display = 'none';
            const placeholder = document.createElement('div');
            placeholder.className = 'bg-gray-200 p-4 text-center text-gray-400 rounded';
  ...
  const renderSpecialContent = () => {
    ...
    if ((campaign?.itemType == 'car' || campaign?.item_type == 'car') && (campaign.car || campaign.item)) {
      return <CampaignCarType campaign={campaign} />;
    }
    if ((campaign?.itemType == 'product' || campaign?.item_type == 'product') && campaign.product) {
      return <CampaignProductType campaign={campaign} />;
    }
    if ((campaign?.itemType == 'real-estate' || campaign?.item_type == 'real-estate' || campaign?.item_type == 'real_estate') && (campaign.real_estate || campaign.realEstate)) {
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <Card className="mb-8">
              ...
              {campaign.link && (
                <div className="px-6 pb-6">
                  ...
                  >
                    <Link href={campaign.link} target="_blank" rel="noopener noreferrer">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ads?.find(item => item.position == 'campaign_content_one') && (
              <Ad position="center" ad={ads.find(item => item.position == 'campaign_content_one')} />
            )}
            {ads?.find(item => item.position == 'campaign_content_two') && (
              <Ad position="center" ad={ads.find(item => item.position == 'campaign_content_two')} />
            )}
            {ads?.find(item => item.position == 'campaign_right') && (
              <Ad position="right" ad={ads.find(item => item.position == 'campaign_right')} />
            )}
          </div>
        </div>
      </div>
