...
import { AlignRight, ChevronDownIcon, ChevronRight,ChevronRightIcon, LogIn, Newspaper, SearchIcon, Settings } from "lucide-react"
...
const loanTypes = [
 ...
 { name: 'Konut Kredisi', href:'/kredi/konut-kredisi', iconKey: 'Ev Yaşam & Ofis' }
]
...
const MobileMenu = () => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
     ...
     {mounted && !isLoggedIn ? <SheetContent className=" max-w-md rounded-t-md px-5" side="bottom">
        
        
        
        <div className="grid grid-cols-3 gap-y-2">
       {loanTypes.map((loan, index) => (<div key={`mobile-loan-${index}`} className="space-y-1">

        <Button asChild className="flex w-full flex-col justify-between items-center gap-2" variant="ghost">
         <Link href={loan.href}>
         <div className="product-des" dangerouslySetInnerHTML={{ __html: getIcon(loan.iconKey) }}></div>
         {loan.name}
         </Link>
        </Button>
       </div>))}
...
function CategoryAccordion({ menuItems = [] }) {
  return (
    <div className="w-full grid  items-center ">
              {Array.isArray(menuItems) && menuItems.map((category) => (
                  ...
                  <AccordionItem value={category.slug}>
                    ...
                    <AccordionContent>
                      <div className="grid gap-y-2">
                        {category.children && Array.isArray(category.children) && category.children.map((children) => (
                          <Link key={children.slug} href={`/kategori/${children.slug}`} className="space-y-1">
                            <Button className="justify-between items-center w-full" variant="ghost">
                              {children.name}
                            </Button>
                          </Link>
                        ))}
  ...
  return (
    ...
    <Dialog>
     ...
     <DialogContent className="sm:max-w-[400px]">
      <DialogHeader>
       ...
       <DialogDescription>Geniş Kampanya Seçeneklerimizi Keşfedin.</DialogDescription>
      </DialogHeader>
      <div className="grid gap-y-2">
       {Array.isArray(menuItems) && menuItems.map(category => (<div  key={category.slug} className="space-y-1">
        <Dialog>
         <div className="flex justify-between items-center">
    <Button asChild className="flex w-full justify-start items-center gap-x-4" variant="ghost">
           <Link href={`/kategori/${category.slug}`}>
            <div className="product-des" dangerouslySetInnerHTML={{ __html: getIcon(category.name) }}></div>
           {category.name}
           </Link>
   ...
   <DialogTrigger asChild>
         ...
         <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
           <DialogTitle>
            <div className="product-des mb-2" dangerouslySetInnerHTML={{ __html: getIcon(category.name) }}></div>
            {category.name}</DialogTitle>
           <DialogDescription>{`${category.name} Geniş Kampanya Seçeneklerimizi Keşfedin.`}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-y-2">
           {category.children && Array.isArray(category.children) && category.children.map(children => (<Link key={children.slug} asChild href={`/kategori/${children.slug}`} className="space-y-1">
            <Button className="justify-between items-center  w-full" variant="ghost">
             {children.name}
    ...
    <Dialog>
     ...
     <DialogContent className="sm:max-w-[400px]">
      <DialogHeader>
       ...
       <DialogDescription>Hesaplama Yapmak İstediğiniz Kredi Türlerini Keşfedin.</DialogDescription>
      </DialogHeader>
      <div className="grid gap-y-2">
       {loanTypes.map((loan, index) => (<div key={`loan-${index}`} className="space-y-1">

        <Button asChild className="flex w-full justify-between items-center gap-2" variant="ghost">
         <Link href={loan.href}>
         <div className="product-des" dangerouslySetInnerHTML={{ __html: getIcon(loan.iconKey) }}></div>
         {loan.name}
  ...
  <header className={`bg-background py-2 md:py-2  dark:bg-gray-950 `}>
   <div className="container flex h-16 items-center justify-between">
    <Link className="flex items-center gap-2 font-semibold" href="/">
     <img className="w-[80px] md:[w-80px]" src="/logo.png" alt="KampanyaRadar"></img>
    </Link>
    <nav className="hidden w-full ml-auto justify-between items-center md:flex">
     ...
     <div className="grid grid-cols-2 items-center">
      ...
      }} className="h-9 gap-x-5 px-4 text-sm font-medium border-none" variant="outline">
       <Newspaper size={20}></Newspaper> Kampanya Oluştur
      </Button>
      {mounted ? (!isLoggedIn ? <AuthDialog></AuthDialog>:<NavbarAvatar size={35} profile={profile}></NavbarAvatar>) : null}
