...
export async function getServerSideProps(context) {
  try {
    const response = await serverApiRequest(`/campaigns/${context.params.slug}`,'get');
...
export default function Campaign({ campaign, categories,ads, isGone }) {
 ...
 useEffect(() => {
   if (typeof window !== 'undefined' && campaign?.lead_form) {
     import('@/components/common/campaign/CampaignForm').then(module => {
 ...
 return (
  <Layout>
   <NextSeo
    ...
    />
  {campaign?.lead_form && CampaignForm && <CampaignForm form={campaign.lead_form} campaignId={campaign.id}></CampaignForm>}
