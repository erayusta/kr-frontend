...
function BrandPaginationBar({ currentPage, lastPage, onPageChange }) {
	...
	const fetchProducts = useCallback(async (pageNum) => {
		...
		try {
			const data = await apiRequest(
				`/marketplace/products?brand_id=${brandId}&per_page=12&page=${pageNum}`,
				"get",
			);
			setProducts(data.data || []);
			setLastPage(data.meta?.last_page || data.last_page || 1);
		} catch (err) {
			console.error("[BrandProducts] Fetch error:", err.message);
